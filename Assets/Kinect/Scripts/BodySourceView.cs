			...
			if( jt == Kinect.JointType.Head ){
				if(flag < 10)flag ++;
					...
					if(!initialized){
						...
						float anglePara = radianPara * 180.0f/Mathf.PI;
						
						
						float radianVert = Mathf.Atan2(headpos.y, headpos.z);
						float angleVert = radianVert * 180.0f/Mathf.PI;
						
						Camera.main.transform.eulerAngles = new Vector3(angleVert, -anglePara, 0);
						//Camera.main.transform.position.x = headpos.x;
						//Camera.main.transform.position.y = headpos.y;
						Camera.main.transform.position = new Vector3(headpos.x, headpos.y-100.0f, -17.0f);
					}
				}
			}
